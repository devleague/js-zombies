...
class Player {
  ...
  takeItem(items) {
    ...
    if (this.getPack().length < 3) {
      this.getPack().push(items)
      console.log((this.name + '      ' + items.name))
      return true
    } else {
  ...
  equip(itemToEquip) {
    let ind = this.getPack().indexOf(itemToEquip);
    if (this.getPack().includes(itemToEquip) && itemToEquip instanceof Weapon && !this.equipped) {
      this.equipped = itemToEquip;
      this.getPack().splice(ind, 1)
    } else if (this.getPack().includes(itemToEquip) && itemToEquip instanceof Weapon && this.equipped) {
      let oldWeapon = this.equipped;
      this.equipped = itemToEquip;
      this.getPack().splice(ind, 1);
      this.getPack().push(oldWeapon);
    }
  }
  ...
  // equip(itemToEquip){
  //  let indW = this.getPack().indexOf(itemToEquip);
  //  if(itemToEquip instanceof Weapon && this.getPack().indexOf > -1 ){

  //   this.equipped = this.itemToEquip
  //  this.getPack().splice(indw,1);
   ...
   */
  // 1.can only eat Food
  // 2.only eat items from pack

  // 3.remove item from package
  // 4 increase players health by food energy amount
  // 5 health cannot exceed max health
  // use max health method to acesss players health
