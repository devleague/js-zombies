...
class Player {
  ...
  takeItem(item) {
    if (this.getPack().length < 3) {
      console.log(this.name + ' picks up the ' + item._name);
  ...
  discardItem(item) {
    ...
    if (this.getPack().indexOf(item) === -1) {
      console.log(this.name + ', the ' + item._name + ' was not found. Nothing was removed from the pack.')
      return false;
    } else {
      this.getPack().splice(itemIndex, 1);
      console.log(this.name + ', the ' + item._name + ' was removed.')
      return true;
    }
  }
  ...
  equip(itemToEquip) {
    ...
    if (itemToEquip instanceof Weapon && itemIndex !== -1) {
      if (this.equipped === false) {
        ...
        this.discardItem(itemToEquip); //if already equipped weapon, replace with current equip
  ...
  eat(itemToEat) {
    ...
    if (itemToEat instanceof Food && foodInPack !== -1) {
      if ((itemToEat.energy + this.health) > this.getMaxHealth()) {
        ...
        console.log('Nom noms! Feeling da best! Health is now: ' + this.health)
      } else if ((itemToEat.health + this.health) <= this.getMaxHealth)
        this.health += itemToEat.energy;
        this.discardItem(itemToEat);
        console.log('Snack attack! Feeling a bit better. Health is now: ' + this.health)
    } else {
      console.log('Not food! No noms!')
    }
  }
  ...
  equippedWith() {
    if (this.equipped !== false) {
      console.log(this.name + ', you have ' + this.equipped.name)
      return this.equipped.name;
    } else {
      console.log(this.name + ', you have nothing equipped.')
      return false;
    }
  }
}; // *** closes Player Class ***
