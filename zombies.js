...
Player.prototype.takeItem = function (item) {
  if (this.getPack().length < 3) {
    console.log(this.name + ' picks up the ' + item.name);
...
Player.prototype.discardItem = function (item) {
  ...
  if (this.getPack().indexOf(item) === -1) {
    console.log(this.name + ', the ' + item.name + ' was not found. Nothing was removed from the pack.')
    return false;
  } else {
    this.getPack().splice(itemIndex, 1);
    console.log(this.name + ', the ' + item.name + ' was removed.')
    return true;
  }
}
